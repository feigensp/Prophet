...
public class HTMLFileView extends JPanel {
	 ...
	 */
	public HTMLFileView(String path, QuestionListPanel cqlp) {
		super();
		data = new QuestionData(path);
		this.cqlp = cqlp;
		textPanes = new ArrayList<JScrollPane>();
		times = new ArrayList<ClockLabel>();
	...
	public void addQuestion(QuestionInfos q) {
		...
		times.add(new ClockLabel(q.getKey()));
		maxPos++;
		curPos++;	
	...
	private void refreshListData() {
		...
		for (int i = 0; i < data.getCategorieCount(); i++) {
			...
			if(questSwitch != null && questSwitch.equals("true")) {
				cqlp.addCategory(questions, ListIcons.UPDOWNARROW);				
			} else {
				cqlp.addCategory(questions, ListIcons.DOWNARROW);				
			}
		}
		cqlp.selectQuestion(data.getLastCategoryIndex(),
				data.getLastQuestionIndex());
