...
public class SyntaxHighlightingPlugin implements CodeViewerPlugin {
	...
	public List<SettingsComponentFactory> getSettingsComponentFactories(QuestionTreeNode selected) {
		...
		if (selected.isCategory()) {
			result.add(new SettingsComponentFactory(selected, new SettingsCheckBox(),"syntaxhighlighting_default", "Syntaxhighlighting einschalten"));
			result.add(new SettingsComponentFactory(selected, new SettingsCheckBox(),"syntaxhighlighting_toggle", "Syntaxhighlighting ein- und ausschaltbar"));
		}
		return result;
	}
}
