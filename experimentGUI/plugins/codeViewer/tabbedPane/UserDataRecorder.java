...
public class UserDataRecorder {
	...
	public ArrayList<FileTime> getTimeLine() {
		timeLine.add(new FileTime(selectedTitle, timeLineWatch.getCurrentTime()));
		return timeLine;
	}
	...
	public void startFileTime(JTabbedPane tabbedPane) {
		clocks = new ArrayList<ClockLabel>();
				...
				if(fileTimesTabbedPane.getComponentCount()>=1) {
					...
					if (lastClock >= 0) {
						...
						fileTimes.get(lastClock).setTime(
								clocks.get(lastClock).getCurrentTime());
					}
					...
					fileTimes.get(lastClock).setTime(clocks.get(lastClock).getCurrentTime());					
				}
			}
		});
