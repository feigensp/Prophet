...
class SyntaxDocument extends DefaultStyledDocument {
	 ...
	 * each line
	 */
	public void processChangedLines(int offset, int length, String str) throws BadLocationException {
		String content = doc.getText(0, doc.getLength());
	...
	private void applyHighlighting(String content, int line) throws BadLocationException {
		...
		String lineContent = content.substring(startOffset, endOffset);
		...
		int singleLineCommentPos = lineContent.indexOf(SINGLE_LINE_COMMENT_DELIMITER);
		while (singleLineCommentPos != -1) {
			if (!doc.getCharacterElement(lineStart + singleLineCommentPos).getAttributes()
					.containsAttributes(quote)) {
				doc.setCharacterAttributes(lineStart + singleLineCommentPos, endOffset
						- (lineStart + singleLineCommentPos), comment, true);
				endOffset = lineStart + singleLineCommentPos;
				break;
			}
			singleLineCommentPos = lineContent.indexOf(SINGLE_LINE_COMMENT_DELIMITER,
					singleLineCommentPos + 2);
		}
